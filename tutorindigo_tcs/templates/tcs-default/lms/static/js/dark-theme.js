$(document).ready(function() {
    ...
    const themeCookie = 'tcs-toggle-dark';

    function applyThemeOnPage(){
      const theme = $.cookie(themeCookie);
      {% if TCS_ENABLE_DARK_TOGGLE %}
      $('body').toggleClass("tcs-dark-theme", theme === 'dark');       // append or remove dark-class based on cookie-value
      // update expiry
    ...
    window.addEventListener("message", function(e){
      if (e.data && e.data["tcs-toggle-dark"]){
        applyThemeOnPage();
      }
    });
